...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from cadastro where email='${login}' and senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    nome = req.body.nome;
    sobrenome = req.body.sobrenome; // depois de .body, use o nome (name) do campo em seu formulário de login
    ...
    if (login == undefined || senha == undefined || nome == undefined || sobrenome == undefined) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${login} / ${senha} / ${nome} / ${sobrenome}`);
    }
    return banco.sql.query(`select count(*) as contagem from cadastro where email = '${login}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into cadastro (email, nome_user,sobre_nome ,senha) values ('${login}','${nome}','${sobrenome}','${senha}')`).then(function() {
